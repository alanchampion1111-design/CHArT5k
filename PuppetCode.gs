...
const gc = {
  ...
  // browserURL: 'https://lyatt-2a01-e0a-1016-fe50-4c71-dad2-aa46-5143.run.pinggy-free.link',  // shared local service (unstable pinggy)
  browserURL: "https://harmless-subarctic-barrier.ngrok-free.dev", // shared local service (stable ngrok for 1GB & 20k requests/mth)
  // browserURL: "https://harmless-subarctic-barrier.eu.ngrok.io",   // legacy supported SSL
...
async function OpenChromeBrowser() {
  const initBrowserURL = 'initBrowser';
  try {
    var response = await FetchNgrok(initBrowserURL);
    browserSession = response.getContentText();   // WS Endpoint lingers for upto 60 minutes
  ...
  timeSecs = 20)
{
  ...
  try {
    var response = await UrlFetchApp.fetch(getBrowserURL, {
      ...
      timeout: timeMax,
      headers: {
        "ngrok-skip-browser-warning": "true" // Keeping ngrok happy
      }
    });
  ...
  timeSecs = 20)
{
  ...
  let timeMax = timeSecs*1000;
  try {
    var response = await FetchNgrok(getFetchURL,
...
async function CloseChromeBrowser() {
  const stopBrowserURL = 'stopBrowser';
  try {
    var response = await FetchNgrok(stopBrowserURL);
...
const allForONE = '/5k/'; // Cache as prev if same location as next member
...
async function AssessPositions(matchRunner,thisUrl,ageCat,genderCat,cacheUrl=false) {
  const filterFetchURL = 'filterUrl'
    +'?url='+thisUrl
...
function TrackImportDate(eventDate) {
  ...
  let prevImportDate = gv.allRunnersSheet
    ...
    .getDisplayValue();   // assumes dd/mm/yyyy format on SS
  if (gc.debug)
...
async function AcceptCookies() {
  const acceptCookiesURL = 'acceptCookies';
  try {
    var response = await FetchNgrok(acceptCookiesURL,
