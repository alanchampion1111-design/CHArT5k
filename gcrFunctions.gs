  ...
  // runnerNameId ='Alan_13')   // for testing
{
  selectPerfCharts = {};
  var csvData = [
    ["Chart Title","Chart Id","Sheet","Sheet Id","Range","Local Runner Ids"]
  ];
    ...
    if (perfSheet) {
      ...
      perfCharts.forEach(perfChart => {
        ...
        if (runnerNameId) {   // filter by runner
          ...
          var localRunnerIds = runnersNames.map((name, i) => {
            if (i % 2 === 1) return name.includes('_')
              ? name
              : name+'_'+runnersNames[i-1];
          }).filter(Boolean).join(',');
          csvData.push([chartTitle,chartId,perfSheetName,perfSheetId,chartRange,'"'+localRunnerIds+'"']);
        } 
      });
    }
  });
  if (runnerNameId) {
    ...
    Logger.log(csv);
    DriveApp.createFile('perfCharts.csv',csv,MimeType.CSV);
  }
}
